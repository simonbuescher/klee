...
namespace klee {
    ...
    ref<klee::ConstantExpr> ADDExecutor::evalConstant(const Constant *c,
                                                   const KInstruction *ki) {
        if (!ki) {
            KConstant* kc = this->kleeModule->getKConstant(c);
            if (kc)
                ki = kc->ki;
        }
        ...
        if (const llvm::ConstantExpr *ce = dyn_cast<llvm::ConstantExpr>(c)) {
            ...
            if (const ConstantInt *ci = dyn_cast<ConstantInt>(c)) {
                ...
                const StructLayout *sl = this->kleeModule->targetData->getStructLayout(cs->getType());
    ...
    ref<ConstantExpr> ADDExecutor::evalConstantExpr(const llvm::ConstantExpr *ce,
                                                 const KInstruction *ki) {
            ...
            case Instruction::GetElementPtr: {
                ...
                for (gep_type_iterator ii = gep_type_begin(ce), ie = gep_type_end(ce);
                    ...
                    if (auto STy = ii.getStructTypeOrNull()) {
                        ...
                        const StructLayout *SL = this->kleeModule->targetData->getStructLayout(STy);
                        base = base->Add(
